...
export function SuccessPage() {
  ...
  const lodging = searchParams.get("lodging"); // 쿼리 파라미터에서 user 데이터를 파싱
  ...
  useEffect(() => {
    if (!user || !lodging || !paymentKey || !product_id || !amount) {
      navigate("/"); // 필요한 정보가 없으면 홈으로 리다이렉트
    } else {
      // 현재 상태를 변경하여 뒤로가기를 방지
      window.history.replaceState(null, document.title, window.location.pathname);
    }
  }, [user, lodging, paymentKey, product_id, amount, navigate]);

  const saveLodgingReservation = async (reservationData) => {
    console.log("예약 데이터 전송 중:", reservationData); // 로그 추가
    try {
      const response = await fetch('http://localhost:8988/payment/saveLodgingReservation', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('숙소 예약 정보를 저장하는데 실패했습니다:', response.statusText);
      }
    } catch (error) {
      console.error('숙소 예약 정보를 저장하는 중 오류 발생:', error);
    }
  };
  ...
  const confirmPayment = useCallback(async () => {
    if (!user) {
      ...
      console.log("숙소: " + lodging);
      console.log("paymentKey: " + paymentKey);
      console.log("orderId:" + product_id);
      console.log("amount:" + amount);
    }
    try {
      const response = await fetch("http://localhost:8988/payment/detailLodging", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
      ...
      if (response.ok) {
        ...
        const lodgingId = parseInt(product_id.replace('order_', ''), 10);
        saveLodgingReservation({
          memberId: parseInt(user, 10), // Long 타입으로 변환
          lodgingId: isNaN(lodgingId) ? 0 : lodgingId,
          relationship: 10001,
  ...
  }, [paymentKey, product_id, amount, user, lodging]);
