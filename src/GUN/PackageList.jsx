    ...
    <p>검색 결과가 없습니다.</p>
                ...
                <div className={styles['restList-right-col']}>
                    <div className={styles['restList-sidebar']}>
                        ...
                        <div className={styles['restList-filter']}>
                            <input type="checkbox" checked={checkboxStates['한식']} onChange={() => toggleCheckbox('KOREAN')} /><p>동남아/대만/서남아</p><span>({data.filter(item => item.restCategory === 'KOREAN').length})</span>
                        </div>
                        <div className={styles['restList-filter']}>
                            <input type="checkbox" checked={checkboxStates['양식']} onChange={() => toggleCheckbox('ITALIAN')} /><p>일본</p><span>({data.filter(item => item.restCategory === 'ITALIAN').length})</span>
                        </div>
                        <div className={styles['restList-filter']}>
                            <input type="checkbox" checked={checkboxStates['중식']} onChange={() => toggleCheckbox('CHINESE')} /><p>유럽/아프리카</p><span>({data.filter(item => item.restCategory === 'CHINESE').length})</span>
                        </div>
                        <div className={styles['restList-filter']}>
                            <input type="checkbox" checked={checkboxStates['일식']} onChange={() => toggleCheckbox('JAPANESE')} /><p>중국/홍콩/몽골/중앙아시아</p><span>({data.filter(item => item.restCategory === 'JAPANESE').length})</span>
                        </div>
                        <div className={styles['restList-filter']}>
                            <input type="checkbox" checked={checkboxStates['불란서식']} onChange={() => toggleCheckbox('FRENCH')} /><p>괌/사이판/호주/뉴질랜드</p><span>({data.filter(item => item.restCategory === 'FRENCH').length})</span>
                        </div>
                        <div className={styles['restList-filter']}>
                            <input type="checkbox" checked={checkboxStates['기타']} onChange={() => toggleCheckbox('ETC')} /><p>미국/하와이/캐나다/중남미</p><span>({data.filter(item => item.restCategory === 'ETC').length})</span>
                        </div>
                    </div>
                </div>
            </div>
